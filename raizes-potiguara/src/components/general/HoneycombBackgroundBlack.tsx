...
const HoneycombBackgroundBlack = () => {
    return (
        <Box position="absolute" inset={0} overflow="hidden" zIndex={0} aria-hidden>
            <Box
                position="absolute"
                inset={0}
                bgImage={`linear-gradient(to bottom, ${CORES.CINZA_ESCURO}, ${CORES.CINZA_ESCURO})`}
            />
            <Box
                position="absolute"
                inset={0}
            ...
            />
            <Box position="absolute" inset={0} bg="black" opacity={0.7} zIndex={0} aria-hidden />
            <Box
                position="absolute"
                inset={0}
                bgImage={`linear-gradient(to bottom, transparent 65%, ${CORES.PRETO} 100%)`}
            />
        </Box>
    );
};

export default HoneycombBackgroundBlack;